...
import type { Post, PostTemplateData, IndexTemplateData, IndexPostData, FormatDateFn, NowFn } from './types.ts';

class DataShaper {
  private formatDate: FormatDateFn;
  private now: NowFn;

  constructor({ formatDate, now }: { formatDate: FormatDateFn; now: NowFn }) {
  ...
  buildIndexData(sortedPosts: Post[]): IndexTemplateData {
    ...
    const acc = sortedPosts.reduce((state: { posts: IndexPostData[]; tagSet: Set<string> }, post) => {
