...
// Local geometry/reference types used internally by this module.
interface Point {
  x: number;
  y: number;
}
...
interface PanZoomAPI {
  clientToViewBox: (cx: number, cy: number) => Point;
  toWorldDelta: (c0x: number, c0y: number, c1x: number, c1y: number) => { dx: number; dy: number };
  getScale: () => number;
}
...
(function () {
  ...
  let allNodeElements: Element[] = [];

  function navigateWithTag(tag: string): void {
  ...
  function setupGraphSearch(
    ...
    function updateSelectedTagsUI(): void {
        ...
        if (resultsDisplay) {
          ...
          resultsDisplay.querySelectorAll('.search-result-tag').forEach(el => {
            el.addEventListener('click', () => {
              ...
              if (tag !== null && !selectedTags.includes(tag)) {
  ...
  function highlightMultiple(ids: string[], nodeRefs: Map<string, NodeRef>): void {
    const idSet = new Set(ids);
    document.querySelectorAll('.graph-node').forEach(el => {
      const nid = el.getAttribute('data-id');
      if (nid !== null && idSet.has(nid)) {
  ...
  // Keyboard navigation
  function setupKeyboardNavigation(nodes: GraphNode[], nodeRefs: Map<string, NodeRef>, positions: Map<string, Point>): void {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
        ...
        const el = allNodeElements[currentFocusIndex];
        if (el) {
  ...
  function toggleHelp(): void {
    const existingOverlay = document.getElementById('graph-help-overlay');
    ...
    helpOverlay.addEventListener('click', (e) => {
      if (e.target === helpOverlay || (e.target as Element).classList.contains('help-close')) {
        helpOverlay.remove();
      }
    });
  }
  ...
  function enablePanZoom(svg: SVGSVGElement, root: SVGGElement, options: PanZoomOptions = {}): PanZoomAPI {
    let scale = 1, tx = 0, ty = 0; let panning = false; let lastX = 0, lastY = 0;
    let dragCandidate = false; const DRAG_THRESHOLD = 4; const pointers = new Map<number, Point>(); let pinch: { s0: number; t0: Point; d0: number; world: Point } | null = null; let lastTapTime = 0;
    let initialScale = 1, initialTx = 0, initialTy = 0;
    function apply(): void { root.setAttribute('transform', `translate(${tx},${ty}) scale(${scale})`); }
    function clientToViewBox(cx: number, cy: number): Point {
    ...
    function toWorldDelta(c0x: number, c0y: number, c1x: number, c1y: number): { dx: number; dy: number } { const p0 = clientToViewBox(c0x, c0y), p1 = clientToViewBox(c1x, c1y); return { dx: (p1.x - p0.x) / scale, dy: (p1.y - p0.y) / scale }; }
    ...
    function tryStartPinch(): void {
      ...
      const mx = (p0.x + p1.x) / 2, my = (p0.y + p1.y) / 2; const dx = p0.x - p1.x, dy = p0.y - p1.y; const d0 = Math.hypot(dx, dy) || 1;
    ...
    function updatePinch(): void {
      ...
      const p0 = pts[0]!, p1 = pts[1]!;
      const mx = (p0.x + p1.x) / 2, my = (p0.y + p1.y) / 2; const dx = p0.x - p1.x, dy = p0.y - p1.y; const d = Math.hypot(dx, dy) || 1;
    ...
    function endInteraction(e: PointerEvent): void { if (pointers.has(e.pointerId)) pointers.delete(e.pointerId); if (pointers.size < 2) pinch = null; if (pointers.size === 0) { dragCandidate = false; panning = false; svg.classList.remove('panning'); } }
    ...
    function fitToContent(padding = 24): void {
  ...
  function renderGlobalGraph(): void {
    const svg = document.getElementById('global-tag-graph') as SVGSVGElement | null; if (!svg) return;
    ...
    fetch('/graph.json?v=' + ts)
      ...
      .then((graph: GraphData | null) => {
        ...
        const positions = new Map<string, Point>(); const nodeRefs = new Map<string, NodeRef>(); const neighbors = new Map<string, Set<string>>(); function addNeighbor(a: string, b: string): void { if (!neighbors.has(a)) neighbors.set(a, new Set()); neighbors.get(a)!.add(b); }
        let idx = 0;
        ...
        // Draw edges
        const linkMap = new Map<string, SVGLineElement>(); const maxWeight = Math.max(1, ...graph.edges.map(e => e.weight || 1));
        graph.edges.forEach(e => {
          ...
          line.setAttribute('x1', String(p1.x)); line.setAttribute('y1', String(p1.y)); line.setAttribute('x2', String(p2.x)); line.setAttribute('y2', String(p2.y)); line.setAttribute('class', 'graph-link'); line.setAttribute('stroke-width', String(0.5 + 1.5 * ((e.weight || 1) / maxWeight))); root.appendChild(line);
        ...
        function updateEdgesForNode(id: string): void {
          const set = neighbors.get(id) || new Set<string>(); set.forEach(nid => { const p1 = positions.get(id), p2 = positions.get(nid); if (!p1 || !p2) return; const key = id < nid ? `${id}|${nid}` : `${nid}|${id}`; const line = linkMap.get(key); if (line) { line.setAttribute('x1', String(p1.x)); line.setAttribute('y1', String(p1.y)); line.setAttribute('x2', String(p2.x)); line.setAttribute('y2', String(p2.y)); } });
        }
        function setNodePosition(id: string, x: number, y: number): void { positions.set(id, { x, y }); const ref = nodeRefs.get(id); if (ref) { ref.circle.setAttribute('cx', String(x)); ref.circle.setAttribute('cy', String(y)); ref.text.setAttribute('x', String(x + ref.radius + 2)); ref.text.setAttribute('y', String(y + 3)); } updateEdgesForNode(id); }
        function moveNodeAndNeighbors(id: string, dx: number, dy: number, factor = 0.25): void { const p = positions.get(id); if (p) setNodePosition(id, p.x + dx, p.y + dy); const neigh = neighbors.get(id); if (neigh) neigh.forEach(nid => { const pn = positions.get(nid); if (pn) setNodePosition(nid, pn.x + dx * factor, pn.y + dy * factor); }); }
        ...
        nodes.forEach(n => {
          ...
          const r = 3 + 8 * (n.count / maxCount); const c = document.createElementNS('http://www.w3.org/2000/svg', 'circle'); c.setAttribute('cx', String(pos.x)); c.setAttribute('cy', String(pos.y)); c.setAttribute('r', String(r)); const t = document.createElementNS('http://www.w3.org/2000/svg', 'text'); t.setAttribute('x', String(pos.x + r + 2)); t.setAttribute('y', String(pos.y + 3)); t.textContent = n.id; g.appendChild(c); g.appendChild(t); root.appendChild(g); nodeRefs.set(n.id, { g, circle: c, text: t, radius: r });
        ...
        function highlight(id: string): void {
          const neigh = neighbors.get(id) || new Set<string>(); const active = new Set([id, ...neigh]);
          root.querySelectorAll('.graph-node').forEach(el => { const nid = el.getAttribute('data-id'); const on = nid !== null && active.has(nid); el.classList.toggle('active', on); el.classList.toggle('dimmed', !on); });
        ...
        function clear(): void { root.querySelectorAll('.graph-node').forEach(el => el.classList.remove('active', 'dimmed')); root.querySelectorAll('.graph-link').forEach(el => el.classList.remove('dimmed')); }
