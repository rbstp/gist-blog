...
// Minimal shapes for graph.json consumed by the small topic graph
...
(function () {
  // Utilities
  function loadScript(src: string, { defer = true }: { defer?: boolean } = {}): Promise<void> {
  ...
  (function setupTheme() {
    const getSystemTheme = (): string => window.matchMedia('(prefers-color-scheme: light)').matches ? 'light' : 'dark';
  ...
  function initTopicGraph(): void {
    ...
    fetch('/graph.json?v=' + (document.body.getAttribute('data-build-ts') || ''))
      ...
      .then((graph: GraphJson | null) => {
        ...
        const nodesPos = new Map<string, Point>();
        ...
        const linkMap = new Map<string, SVGLineElement>();
        ...
        enablePanZoomSmall(svg as unknown as SVGSVGElement, root);
      })
      .catch(() => { /* ignore */ });
  }

  // Simple pan/zoom used by the small topic graph
  function enablePanZoomSmall(svg: SVGSVGElement, root: SVGGElement): void {
  ...
  const apply = () => root.setAttribute('transform', `translate(${tx},${ty}) scale(${scale})`);
    ...
    function tryStartPinch(): void {
      ...
      const p0 = pts[0], p1 = pts[1]; if (!p0 || !p1) return;
    ...
    function endInteraction(e: PointerEvent): void {
  ...
  function initIndexFilteringAndPagination(): void {
    ...
    function updateFilterStatus(): void {
      ...
      filterStatus.querySelectorAll('.clickable-filter-tag').forEach(el => {
        el.addEventListener('click', () => {
          const tagName = el.getAttribute('data-tag'); const idx = tagName != null ? activeTags.indexOf(tagName) : -1;
    ...
    document.querySelectorAll('.tag').forEach(tagEl => {
      tagEl.addEventListener('click', () => {
        const tagName = tagEl.getAttribute('data-tag'); const idx = tagName != null ? activeTags.indexOf(tagName) : -1;
        if (idx > -1) { activeTags.splice(idx, 1); document.querySelectorAll('.tag').forEach(t => { if (t.getAttribute('data-tag') === tagName) t.classList.remove('active'); }); }
        else if (tagName != null) { activeTags.push(tagName); document.querySelectorAll('.tag').forEach(t => { if (t.getAttribute('data-tag') === tagName) t.classList.add('active'); }); }
        currentPage = 1; updateFilterStatus(); updatePagination();
      });
    });
    ...
    (function applyPreselectedTags() {
      let preload: unknown = null; try { preload = JSON.parse(localStorage.getItem('preselectedTags') || 'null'); } catch { preload = null; }
      if (!Array.isArray(preload) || preload.length === 0) return;
      const set = new Set(preload); const allTags = document.querySelectorAll<HTMLElement>('.tag'); allTags.forEach(t => { const name = t.getAttribute('data-tag'); if (name != null && set.has(name)) t.classList.add('active'); });
      preload.forEach((name: unknown) => { const el = Array.from(allTags).find(e => e.getAttribute('data-tag') === name); if (el) el.click(); });
  ...
  function initTocBehaviors(): void {
    ...
    function ensureVisible(link: Element | null, center = false): void {
